...
export const PMSelector: React.FC<PMSelectorProps> = ({ pms, selected, onChange }) => {
  ...
  useEffect(() => {
    const handleClose = (e: MouseEvent) => {
      if (
        ...
        !(e.target as Element).closest('[data-pm-dropdown]')
      ) {
        setOpen(false);
      }
    };
  ...
  const filtered = pms.filter((pm) => pm.toLowerCase().includes(search.toLowerCase()));
  const allSelected = selected.length === pms.length && pms.length > 0;
  const noneSelected = selected.length === 0;

  const toggle = (pm: string) => {
    onChange(selected.includes(pm) ? selected.filter((s) => s !== pm) : [...selected, pm]);
  };
          ...
          return (
            <li
              ...
              className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors text-sm ${
                isSelected ? 'bg-vois-50 text-vois-800' : 'text-gray-700 hover:bg-gray-50'
              }`}
            >
              <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                isSelected ? 'bg-vois-600 border-vois-600' : 'border-gray-300 bg-white'
              }`}>
                {isSelected && (
    ...
    <div className="w-full" ref={containerRef}>
      ...
      <div className="flex flex-wrap gap-2 mb-3 min-h-[36px]">
        ...
        {selected.slice(0, 5).map((pm) => (
          <span key={pm} className="inline-flex items-center gap-1 px-3 py-1 bg-vois-100 text-vois-800 rounded-full text-xs font-medium">
            {pm}
            <button onClick={() => toggle(pm)} className="hover:text-vois-600 ml-0.5 text-vois-500">×</button>
          </span>
        ))}
        {selected.length > 5 && (
